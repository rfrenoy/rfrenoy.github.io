...
The system prompt instructs Claude to look at the screenshots and extract the color palette, layout patterns, and key features, then generate a single self-contained React component (`ProductDemo.tsx`) that renders an animated 30-second video.

The video follows a fixed structure:
- <mark>Intro (0–4s)</mark>: Product name with an entrance animation
- <mark>Feature showcase (4–24s)</mark>: Split-screen scenes — animated UI mockups on one side, value proposition text on the other. Elements appear sequentially (sidebar slides in, cards fade in, buttons appear)
- <mark>Tagline (24–30s)</mark>: Value proposition, centered, with a clean transition
...
After building the CLI, I turned it into a web product: [getproductdemo.app](https://getproductdemo.app). Upload your screenshots, enter your product name and tagline, and get a video delivered to your inbox in a couple of minutes. No API key needed, no local setup.

The web version adds a few things on top of the CLI: Stripe payments, email delivery of the finished video (so you can close the tab while it renders), automatic refunds if generation fails, and a job queue for handling concurrent requests.

If you'd rather run it yourself, the [CLI is open source](https://github.com/rfrenoy/auto-product-demo) and works with your own Anthropic API key.
...
*The project is built with TypeScript, [Remotion](https://remotion.dev), and the [Anthropic SDK](https://docs.anthropic.com). The CLI runs entirely locally. The web version is hosted at [getproductdemo.app](https://getproductdemo.app).*
